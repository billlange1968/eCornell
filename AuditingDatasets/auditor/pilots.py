...
def get_certification(takeoff,student):
    ...
    last=student[1]
    first=student[2]
    joined=utils.str_to_time(student[3])
    solo=utils.str_to_time(student[4])
    license=utils.str_to_time(student[5])
    hours50=utils.str_to_time(student[6])
    instrument=utils.str_to_time(student[7])
    advanced=utils.str_to_time(student[8])
    multiengine=utils.str_to_time(student[9]) 

    if hours50 != None and takeoff.date() >= hours50.date():
        return PILOT_50_HOURS
    elif license != None and takeoff.date() >= license.date():
        return PILOT_CERTIFIED
    elif solo != None and takeoff.date() >= solo.date():
        return PILOT_STUDENT
    elif joined != None and takeoff.date() >= joined.date():
        return PILOT_NOVICE
    else:
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    for row in minimums: 
        ...
        if t_area == area:
            match.append(area)
        elif t_area == 'Local' and (area == 'Pattern' or area == 'Practice Area'):
            match.append(area)
        elif t_area == 'Any':
            match.append(area)           
        else:
    ...
    if len(table)==0:
        ...
        r_wind = get_best_value(table, 2)
        r_crosswind = get_best_value(table, 3)
